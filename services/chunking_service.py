...
class ChunkingService:
    ...
    def _apply_semantic_split(self, chunks_list, content, headers, tenant_id, src_file, access_role):
        try:
            ...
            for sub in sub_docs:
                ...
                if t_count > HARD_CAP:
                    ...
                    for hard_txt in hard_splits:
                        self._add_chunk(chunks_list, hard_txt, headers, tenant_id, src_file, access_role, "hard_cap_fallback")
                else:
                    self._add_chunk(chunks_list, sub.page_content, headers, tenant_id, src_file, access_role, "hybrid_semantic")
                    
        except Exception as e:
            hard_splits = self.fallback_splitter.split_text(content)
            for hard_txt in hard_splits:
                self._add_chunk(chunks_list, hard_txt, headers, tenant_id, src_file, access_role, "hard_cap_error_fallback")

    def process_hybrid_splitting(self, text: str, tenant_id: str, src_file: str, access_role: List[int]) -> List[Dict]:
        """
        ...
        for doc in merged_splits:
            ...
            # Case A: Tokens of Chunk > MAX_TOKENS -> Semantic Split
            if token_count > MAX_TOKENS:
                self._apply_semantic_split(final_chunks, content, headers, tenant_id, src_file, access_role)
            
            # Case B: Tokens of Chunk <= MAX_TOKENS -> Save
            else:
                method = "markdown_adaptive" if token_count > MIN_TOKENS else "merged_small"
                self._add_chunk(final_chunks, content, headers, tenant_id, src_file, access_role, method)

        return final_chunks

    def _add_chunk(self, chunks_list, content, headers, tenant_id, src_file, access_role, method):
